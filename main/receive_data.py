...
def receive_CAN(d_name, DATASET_PATH, P_db, C_db, can_bus, stop_event):
    ...
    # P_msg_list = ["CAN MESSAGE NAME HERE"]
    C_msg_list = ["CAN MESSAGE NAME HERE"]
    ...
    can_monitoring = {'CAN ID HERE': -1, 'CAN ID HERE': -1, 'CAN ID HERE': -1}
    cnt = 0
    first = True
    time_total = 0.
    cycle = 0
            ...
            for msg in db_msg:
                if can_msg.arbitration_id == msg.frame_id:
                    ...
                    if msg.name == 'CAN MESSAGE NAME HERE':
                        can_monitoring['CAN MESSAGE NAME HERE'] = can_dict['CAN ID HERE']
                    elif msg.name == 'CAN MESSAGE NAME HERE':
                        can_monitoring['CAN MESSAGE NAME HERE'] = can_dict['CAN ID HERE']
                    elif msg.name == 'CAN MESSAGE NAME HERE':
                        can_monitoring['CAN MESSAGE NAME HERE'] = can_dict['CAN ID HERE']
    ...
    try:
        while True:
            ...
            if sideView:
                ...
                if not depth_frame_2 or not color_frame_2 or not ir_frame_1:
                    print("[INFO] VIDEO FRAME ERROR")
                    continue
...
def receive_audio(d_name, DATASET_PATH, FORMAT, RATE, CHANNELS, CHUNK, stop_event):
    ...
    sync_process()
    print(f"[INFO] '{d_name}' process starts collecting.")
